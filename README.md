...
- 🌍 **全球新闻源**：支持路透社、美联社、BBC、Hacker News 等 12 个主流媒体
- 🔄 **自动翻译**：支持 OpenAI、DeepL、Google、百度等多个翻译服务
- ✅ **真实性验证**：
  - 可信度评估（基于来源、内容完整性、语言特征）
  - 事实核查（检查消息来源和具体事实）
  - 交叉引用验证（检查多源报道）✨ 新功能
- 🤖 **自动定时任务**：按优先级自动抓取、翻译、验证 ✨ 新功能
- 🌐 **代理支持**：支持 HTTP/HTTPS/SOCKS5 代理，突破网络限制 ✨ 新功能
...
定时任务包括：
- **高优先级新闻**（Reuters、AP、BBC、Bloomberg、HackerNews、AFP）：每小时抓取
- **中优先级新闻**（Guardian、NYT、AlJazeera、TechCrunch）：每6小时抓取
- **低优先级新闻**（联合早报等）：每天凌晨2点抓取
- **自动翻译**：每30分钟处理待翻译新闻
- **自动验证**：每小时处理待验证新闻
- **清理旧新闻**：每周日凌晨3点清理30天前的新闻
