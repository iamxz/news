...
class ToutiaoFetcher(BaseFetcher):
    ...
    async def fetch(self) -> List[NewsArticle]:
        ...
        try:
            ...
            response = self._make_request(self.hotsearch_api, headers=headers, check_robots=False)
            if not response:
            ...
            try:
                ...
                if 'data' in data:
                    if isinstance(data['data'], dict):
                        ...
                        logger.info(f"data 字段类型: {type(data['data'])}，长度: {len(data['data']) if hasattr(data['data'], '__len__') else 'N/A'}")
                    ...
                    for path in search_paths:
                        ...
                        for key in path:
                            ...
                            if isinstance(current, list) and len(current) > 0:
                                hot_list = current
                                logger.info(f"从路径 {path} 中找到 {len(hot_list)} 条热搜")
                                break
                ...
                if hot_list:
                    ...
                    for item in hot_list:
                        ...
                        hot_score = item.get('hotValue', '') or item.get('HotValue', '')
                        
                        # 尝试其他可能的字段名
                        if not title:
                            title = item.get('name', '').strip() or item.get('Name', '').strip()
                        if not url:
                            url = item.get('link', '') or item.get('Link', '')
                        if not hot_score:
                            hot_score = item.get('hot', '') or item.get('Hot', '')
